...
class SynchronousTimeEdgeCasesTests: XCTestCase {
    var listeningItems: [ListeningTime] = []
    override func setUp() {
        guard let listeningTimes = readListeningTimesJson(file: "listeningTimesTest5", bundle: Bundle(for: type(of: self)))?.times else {
    ...
    func testTotalSynchronousTimeCalculation() {
        ...
        XCTAssertEqual(totalListeningTime, 6.0, "Total listeniung times is incorreect")
    }
